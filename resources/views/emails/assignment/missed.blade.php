<html>
<body>
<p>Sehr geehrter Teilnehmer {!! $PatientName !!},</p>
<p>Ihr letzter Schreibimpuls wurde nicht beantwortet und die Bearbeitungsfrist ist leider abgelaufen.</p>
<p>
    Da wir  diesen Schreibimpuls als wichtig erachten, wollen wir Ihnen gerne die Möglichkeit zur Bearbeitung geben und
    haben Ihnen daher den Schreibimpuls für die Folgewoche erneut zugewiesen.
</p>
<p>Bitte besuchen Sie an Ihrem nächsten Schreibtag (am {!! $AssignmentDay !!}, dem {!! $NextWritingDate !!})
    die <a href="{{ url("/") }}">Webseite von GSA-Online plus</a>, um sich Ihren aktuellen Schreibimpuls
    anzusehen und einen Tagebucheintrag zu verfassen.</p>
<p>Viele Grüße,<br/>
    Ihr Team von GSA-Online plus</p>
</body>
</html>
